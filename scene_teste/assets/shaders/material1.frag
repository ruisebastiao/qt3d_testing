...
uniform float unf23;
uniform vec3 unf24;
const float cons29 = float(1.000000000000);
const vec4 cons33 = vec4(1.000000000000, 1.000000000000, 1.000000000000, 0.004999999888);
const float cons42 = float(0.500000000000);
uniform float unf43;
uniform vec3 unf48;
const float cons58 = float(1.000000000000);
uniform vec4 unf59;
uniform vec3 unf61;
const float cons69 = float(1.000000000000);
uniform float unf72;
uniform float unf73;
uniform float unf74;
uniform float unf75;
...
void main()
{
	...
	set_rgb_zero(tmp8);
	set_rgb_zero(tmp9);
	lamp_visibility_sun_hemi(unf10, tmp11, tmp12, tmp13);
	shade_inp(tmp5, tmp11, tmp16);
	shade_is_hemi(tmp16, tmp18);
	shade_visifac(tmp18, tmp13, unf21, tmp22);
	shade_mul_value_v3(unf23, unf24, tmp25);
	shade_mul_value(tmp22, vec4(tmp25, 1.0), tmp28);
	mtex_value_invert(cons29, tmp30);
	mix_mult(tmp30, tmp28, cons33, tmp34);
	shade_madd(vec4(tmp8, 1.0), tmp34, vec4(tmp2, 1.0), tmp38);
	shade_hemi_spec(tmp5, tmp11, tmp7, cons42, unf43, tmp13, tmp45);
	shade_add_spec(tmp45, tmp25, unf48, tmp49);
	shade_add_clamped(vec4(tmp9, 1.0), vec4(tmp49, 1.0), tmp52);
	shade_clamp_positive(tmp52, tmp54);
	shade_clamp_positive(tmp38, tmp56);
	shade_maddf(tmp56, cons58, unf59, tmp60);
	shade_add_mirror(unf61, tmp3, tmp60.rgb, tmp64);
	shade_add(vec4(tmp64, 1.0), tmp54, tmp67);
	mtex_alpha_to_col(tmp67, cons69, tmp70);
	shade_mist_factor(varposition, unf72, unf73, unf74, unf75, unf76, tmp77);
	mix_blend(tmp77, tmp70, unf80, tmp81);
	shade_alpha_opaque(tmp81, tmp83);
